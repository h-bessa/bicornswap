...
contract PantherToken is BEP20 {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            ...
            { 
                uint256 currentSupply = totalSupply().sub(balanceOf(0x000000000000000000000000000000000000dEaD));
                uint256 fee;
                uint256 txSize = amount.mul(10000).div(currentSupply);
                ...
                if (txSize <= 10000) {
                    fee = 0;
                } else if (txSize <= 30000) {
                    fee = 15;
                } else if (txSize <= 50000) {
                    fee = 20;
                } else if (txSize < 70000) {
                    fee = 30;
                } else {
                    fee = 100;
                }
